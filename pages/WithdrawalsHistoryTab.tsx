...
import { Loader2, ChevronRight, ArrowUpRight, CheckCircle2, XCircle, Clock } from 'lucide-react';

const WithdrawalsHistoryTab = ({ user, setActiveTab }: { user: User | null, setActiveTab: (t: string) => void }) => {
    ...
    return (
        <div className="pt-12 px-5 pb-24 animate-slide-up">
            <div className="flex items-center gap-4 mb-8">
                ...
                <h1 className="text-2xl font-bold text-white">سجل السحب</h1>
            </div>
            ...
            <div className="space-y-3">
                {loading ? (
                    ...
                    <div className="text-center py-10 bg-[#141414] border border-[#2A2A2A] rounded-2xl text-gray-500">
                        ...
                        <p className="text-xs">لا توجد عمليات سحب</p>
                    </div>
                ) : (
                    transactions.map((item, idx) => (
                        <div key={idx} className="bg-[#141414] border border-[#2A2A2A] rounded-2xl p-4 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <div className="text-sm font-bold text-white">{item.bank_name}</div>
